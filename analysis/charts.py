...
def kbar_feature_summary(enriched: pd.DataFrame, title: str = "") -> plt.Figure:
    """
    Shows BB%B, momentum_3, and prev_3_green for enriched immediate_loss trades.
    Displays a placeholder when no K-bar data is available.
    """
    df = enriched.dropna(subset=["bb_pct_b"])
    if df.empty:
        fig, ax = plt.subplots(figsize=(6, 3))
        ax.text(0.5, 0.5,
                "No K-bar data available.\nExport full OHLCV from TradingView to enable this chart.",
    ...
    # BB%B
    axes[0].hist(df["bb_pct_b"], bins=15, color=_LOSS, edgecolor="white")
    axes[0].axvline(50, color="gray", linestyle="--", linewidth=1, label="BB midline")
    axes[0].axvline(100, color="orange", linestyle="--", linewidth=1, label="BB upper")
    axes[0].set_title("BB%B at Entry")
    axes[0].set_xlabel("BB%B (0=lower band, 100=upper band)")
